...
const HomePage = ({ user, setUser }) => {
  ...
  return (
    <PatientLayout user={user}> {/* Pass user to PatientLayout */}
      ...
      <div className="relative w-full flex flex-col items-center p-8 bg-teal-50">
        <div className="flex w-8/12 m-auto justify-between items-center mb-4">
          <button
            ...
            className="p-2 bg-teal-500 text-white rounded-full shadow-md"
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-14 flex-1">
            {departments.slice(startIndex, startIndex + visibleCount).map((dept, index) => (
              <div
                key={index}
                className="flex flex-col items-center m-auto p-6 w-32 h-32 bg-white rounded-xl shadow-lg border hover:shadow-2xl cursor-pointer transition duration-300"
          ...
          <button
            onClick={handleNext}
            disabled={startIndex + visibleCount >= departments.length}
            className="p-2 bg-teal-500 text-white rounded-full shadow-md"
          >
