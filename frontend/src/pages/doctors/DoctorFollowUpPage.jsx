...
import { FaCalendarAlt, FaClock, FaUserMd, FaHospital, FaStethoscope } from 'react-icons/fa';
...
const DoctorFollowUpPage = () => {
  ...
  const handleAccept = async (followUpId) => {
    try {
      await axios.post(
        `http://localhost:5000/api/followups/${followUpId}/accept`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success('Follow-up request accepted successfully!', { autoClose: 3000 });
      setFollowUps((prev) =>
        prev.map((fu) => (fu.id === followUpId ? { ...fu, status: 'approved', responseMessage: null } : fu))
      );
    } catch (error) {
                ...
                >
                  ...
                  <div className="w-1/5 flex flex-col space-y-2 items-end">
                    {followUp.status === 'pending' && (
                      <>
                        <button
                          onClick={() => handleAccept(followUp.id)}
