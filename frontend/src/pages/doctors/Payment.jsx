...
import { FaCalendarTimes, FaUser, FaCalendarAlt, FaClock, FaUserMd, FaArrowLeft, FaArrowRight, FaCalendar, FaWallet } from 'react-icons/fa';
...
const Payment = () => {
    ...
    const [wallet, setWallet] = useState({ balance: 0, remainBalance: 0, withdrawalHistory: [] });
    ...
    const fetchWallet = async () => {
        try {
            ...
            setWallet({
                ...
                remainBalance: Number(response.data.data.remainBalance) || 0,
    ...
    const handleWithdraw = async () => {
        console.log('Withdrawal Attempt:', { withdrawalAmount, remainBalance: wallet.remainBalance });
        ...
        if (Number(withdrawalAmount) > wallet.remainBalance) {
            console.log('Withdrawal exceeds remainBalance');
            toast.warn('Withdrawal amount exceeds remaining balance.', { autoClose: 3000 });
            return;
        }
    ...
    const filteredAppointments = appointments.filter((appt) => {
        const matchName = `${appt.Patient?.firstName} ${appt.Patient?.lastName}`.toLowerCase();
            ...
            <div className="p-2 max-w-7xl mx-auto">
                        ...
                        <div className="flex space-x-2 bg-gray-100 p-1 rounded-lg shadow-sm">
                            {['all', 'physical', 'online'].map((type) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-md font-medium transition duration-200 ${typeFilter === type
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-3">
                    ...
                    <div className="bg-white p-6 border border-gray-200 rounded-lg shadow-sm flex items-center space-x-4">
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-gray-500">Remaining Earnings</h3>
                            <p className="text-2xl font-semibold text-gray-800">₹{wallet.remainBalance.toFixed(2)}</p>
                        </div>
                    </div>
                    <div className="bg-white p-6 border border-gray-200 rounded-lg shadow-sm flex items-center space-x-4">
                    ...
                    <div className="bg-white p-6 border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <button
                                ...
                                className="bg-teal-600 text-white px-2 py-2 rounded-lg hover:bg-teal-700 transition duration-200 font-medium shadow-sm"
                            >
                                Withdraw
                            </button>
                        </div>
                        <p className="text-xs text-gray-500 mt-2">Note: A 15% service fee will be deducted.</p>
                        <p className="text-xs text-gray-500 mt-1">Available for withdrawal: ₹{wallet.remainBalance.toFixed(2)}</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-md border border-gray-400">
                    <div className="flex border-b border-gray-200">
                        <button
                            className={`px-6 py-3 font-medium text-sm ${activeTab === 'withdrawals'
                        ...
                        <button
                            className={`px-6 py-3 font-medium text-sm ${activeTab === 'payments'
                                    ? 'border-b-2 border-teal-600 text-teal-600'
                                    : 'text-gray-500 hover:text-gray-700'
                                }`}
                                        ...
                                        <div className="flex space-x-2">
                                            ...
                                            {Array.from({ length: paymentTotalPages }, (_, i) => i + 1).map((page) => (
                                                <button
                                                    ...
                                                    className={`px-3 py-1 rounded-lg ${paymentPage === page
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full table-auto">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Date</th>
                                            <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Amount Requested</th>
                                            <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Deduction (15%)</th>
                                            <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Amount Received</th>
                                        ...
                                        {currentWithdrawalRows.map((withdrawal) => (
                                            <tr key={withdrawal.id} className="hover:bg-gray-50 transition duration-150">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span
                                                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${withdrawal.status === 'completed'
                                    ...
                                    <div className="flex space-x-2">
                                        ...
                                        {Array.from({ length: withdrawalTotalPages }, (_, i) => i + 1).map((page) => (
                                            <button
                                                ...
                                                className={`px-3 py-1 rounded-lg ${withdrawalPage === page
                                                        ? 'bg-teal-600 text-white'
                                                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                                    }`}
                                            >
